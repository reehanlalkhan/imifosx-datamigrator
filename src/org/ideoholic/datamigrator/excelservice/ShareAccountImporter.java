...
public class ShareAccountImporter implements Constants {
	...
	public void importShareData() throws ParseException, ClassNotFoundException, SQLException {
		...
		BigDecimal savings_account_id = getSavingsId(client_id) ; 
	...
	//	insertshareaccountcertificate(share_account_id, share_transaction_id,cert_num, from_num, to_num);
...
private void updateshareproduct(int totalsubscribed_shares,int shares) {
	String sql = "UPDATE m_share_product SET totalsubscribed_shares = "+"'"+totalsubscribed_shares+"'"+" WHERE totalsubscribed_shares ="+"'"+shares+"'";
	try {
...
private void insertshareaccounttransaction(BigDecimal share_account_id, String transaction_date, int total_shares,
	...
	String sql ="insert into m_share_account_transactions\n" + 
			...
			"VALUES('"+share_account_id+"','"+transaction_date+"','"+total_shares+"','"+unit_price+"','"+amount+"',"
					+ "'"+charge_amount+"','"+amount_paid+"','"+status_enum+"','"+type_enum+"','"+is_active+"')";
		
	DBUtils.getInstance().executePreparedStatement(sql); 	
}

private void insertshareaccountcertificate(BigDecimal share_account_id, int share_transaction_id, int cert_num, int from_num,
			int to_num) throws SQLException, ClassNotFoundException {
...
private void insertShareAccount(String account_no, int product_id, BigDecimal client_id, int status_enum,
	...
	String sql = "insert into m_share_account(account_no,product_id, client_id,  status_enum,"
			...
			+ "minimum_active_period_frequency_enum, lockin_period_frequency_enum)values('"+account_no+"','"+product_id+"',"
					+ "'"+client_id+"','"+status_enum+"','"+total_approved_shares+"','"+submitted_date+"','"+submitted_userid+"',"
							+ "'"+approved_date+"','"+approved_userid+"','"+activated_date+"','"+activated_userid+"','"+currency_code+"',"
...
public ArrayList<Integer> getfromshareaccountcertificate() throws ClassNotFoundException, SQLException {
	...
	if (result.next()) {
		...
		throw new SQLException(
				"No share_transaction_id,cert_num");
	}
...
public BigDecimal getSavingsId(BigDecimal client_id) throws ClassNotFoundException, SQLException {
	...
	if (result.next()) {
		...
		throw new SQLException(
				"no savings ID got generated for client id::" + client_id);
	}
...
public BigDecimal getShareId(String account_no) throws ClassNotFoundException, SQLException {
	...
	if (result.next()) {
		...
		throw new SQLException(
				"no Share ID got generated for  Share Account Number"
						+ account_no);
	}
